...
ATCA_STATUS hal_i2c_init(void *hal, ATCAIfaceCfg *cfg) {
  ...
  if (bus >= 0 && bus < MAX_I2C_BUSES) {
    if (i2c_hal_data[bus] == NULL) {
      ...
      //            ESP_LOGD(TAG, "I2C Driver Install; %s",
      //            esp_err_to_name(rc));
...
ATCA_STATUS hal_i2c_send(ATCAIface iface, uint8_t *txdata, int txlength) {
  ...
  txdata[0] =
      0x03; // Word Address value, Command Token as per datasheet of ATECC508A
  txlength++;
  ...
  (void)i2c_master_write_byte(
      cmd, cfg->atcai2c.slave_address | I2C_MASTER_WRITE, ACK_CHECK_EN);
  (void)i2c_master_write(cmd, txdata, txlength, ACK_CHECK_EN);
  (void)i2c_master_stop(cmd);
  rc = i2c_master_cmd_begin(0, cmd, 10);
...
ATCA_STATUS hal_i2c_receive(ATCAIface iface, uint8_t *rxdata,
  ...
  (void)i2c_master_start(cmd);
  (void)i2c_master_write_byte(cmd, cfg->atcai2c.slave_address | I2C_MASTER_READ,
                              ACK_CHECK_EN);
...
ATCA_STATUS hal_i2c_wake(ATCAIface iface) {
  ...
  (void)i2c_master_stop(cmd);
  (void)i2c_master_cmd_begin(0, cmd, 10);
  (void)i2c_cmd_link_delete(cmd);

  atca_delay_ms(10); // wait tWHI + tWLO which is configured based on device
